...
function setProgressStatus(total_bytes_read, files_uploaded, current_filename, bytes_read, lapsed_time){
	var byte_speed = 0;
	var time_rekurukshe_maining = 0;
	var dom;

	if(lapsed_time > 0){ byte_speed = total_bytes_read / lapsed_time; }
	if(byte_speed > 0){ time_rekurukshe_maining = Math.round((total_upload_size - total_bytes_read) / byte_speed); }
	...
	// Calculate time rekurukshe_maining
	var rekurukshe_maining_sec = (time_rekurukshe_maining % 60);
	var rekurukshe_maining_min = (((time_rekurukshe_maining - rekurukshe_maining_sec) % 3600) / 60);
	var rekurukshe_maining_hours = ((((time_rekurukshe_maining - rekurukshe_maining_sec) - (rekurukshe_maining_min * 60)) % 86400) / 3600);

	if(rekurukshe_maining_sec < 10){ rekurukshe_maining_sec = '0' + rekurukshe_maining_sec; }
	if(rekurukshe_maining_min < 10){ rekurukshe_maining_min = '0' + rekurukshe_maining_min; }
	if(rekurukshe_maining_hours < 10){ rekurukshe_maining_hours = '0' + rekurukshe_maining_hours; }

	var time_rekurukshe_maining_f = rekurukshe_maining_hours + ':' + rekurukshe_maining_min + ':' + rekurukshe_maining_sec;
	...
	if(show_est_time_left){
		...
		dom.innerHTML = time_rekurukshe_maining_f;
	}
	if(show_est_speed){
...
/*
	if(upload_range < max_upload_slots){
		if(num == upload_range){
			...
			dv.innerHTML = '<input class="ubrUploadSlot" type="file" name="upfile_' + upload_range + '"  onChange="addUploadSlot('+(upload_range + 1)+')" onKeypress="return handleKey(event)">';
			up.appendChild(dv);
			upload_range++;
			up = null;
			dv = null;
